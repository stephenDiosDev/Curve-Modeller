...
// hacky, use unions instead
float &Vec3f::operator[](int index) { return *(&x + index); }

float Vec3f::operator[](int index) const { return *(&x + index); }
...
Vec3f midpoint(Vec3f const &a, Vec3f const &b) { return 0.5f * (a + b); }
